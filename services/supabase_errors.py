...
def format_supabase_error(exc: Exception) -> str:
    ...
    if "PGRST205" in raw or "Could not find the table" in raw:
        return (
            ...
            "Administrator migratsiyalarni (004_access_requests.sql va boshqalar) ishga tushirishi kerak."
        )
    if "invalid input syntax for type uuid" in raw:
        return "Hisob identifikatori noto'g'ri. Qayta Telegram orqali kiring."
    if len(raw) > 240:
        return "Saqlashda xatolik yuz berdi. Keyinroq qayta urinib ko'ring yoki administratorga murojaat qiling."
    return raw
